        ...
        for kdim in kdim_arr:
            ...
            for inp_wl, inp_sgn, wgt_wl, wgt_sgn in zip(inp_wl_arr, inp_wl_sgn, wgt_wl_arr, wgt_wl_sgn):
                ...
                for s,p in zip(simd, pe):
                    ...
                    if(config_set == 0):
                        with open("config_log.txt","w"):
                            f.write(f'Config set: {config_set}')
                    else:
                        with open("config_log.txt","a"):
                            f.write(f'Config set: {config_set}')
...
if __name__ == '__main__':
    ...
    ifm_ch_arr  = np.array([64,64,64,64,64,64])
    ofm_ch_arr  = np.array([2,4,8,16,32,64])
    ifm_dim_arr = np.array([32,32,32,32,32,32])

    ### Keep the length of the following two arrays same
    inp_wl_arr  = np.array([1,4])
    inp_wl_sgn  = np.array([0,0,0]) ## 0 for unsigned and 1 for signed
    wgt_wl_arr  = np.array([1,1])
